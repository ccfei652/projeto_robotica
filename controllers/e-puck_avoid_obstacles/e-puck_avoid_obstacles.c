...
int main(int argc, char **argv) {
  ...
  wb_motor_set_velocity(MotorDireito,0);
    ...
    WbNodeRef c1 = wb_supervisor_node_get_from_def("caixa1");
    WbNodeRef c2 = wb_supervisor_node_get_from_def("caixa2");
    WbNodeRef c3 = wb_supervisor_node_get_from_def("caixa3");
    WbNodeRef c4 = wb_supervisor_node_get_from_def("caixa4");
    WbNodeRef c5 = wb_supervisor_node_get_from_def("caixa5");
    WbNodeRef c6 = wb_supervisor_node_get_from_def("caixa6");
    WbNodeRef c7 = wb_supervisor_node_get_from_def("caixa7");
    WbNodeRef c8 = wb_supervisor_node_get_from_def("caixa8");
    WbNodeRef c9 = wb_supervisor_node_get_from_def("caixa9");
    WbNodeRef c10 = wb_supervisor_node_get_from_def("caixa10");
    
    if (c1 == NULL) {
        printf("Erro: Falha ao obter o nó c1.\n");
        return 1; 
    }
  ...
  while (wb_robot_step(TIME_STEP) != -1) {
    ...
    while(!mudou){
     ...
     printf("Posicao Antiga: %lf %lf %lf", c1p[0],c1p[1],c1p[2]);
     printf("\nNova Posicao:   %lf %lf %lf", c1n[0],c1n[1],c1n[2]);
     printf("\nPosicao Mudou? %s \n \n\n", mudou ? "Sim" : "Não");
     printf("\n");
     printf("\n");
     printf("\n");
